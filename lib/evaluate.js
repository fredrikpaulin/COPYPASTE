...
async function kFoldCV(task, data, { k = 5, algorithm = 'logistic_regression', onFold } = {}) {
  const { writeJsonl, readJsonl } = await import('./data.js')
  ...
  for (let f = 0; f < folds.length; f++) {
    ...
    const trainPath = join('data', `${task.name}_cv_train_${f}.jsonl`)
    const valPath = join('data', `${task.name}_cv_val_${f}.jsonl`)
    ...
    const outputDir = join('models', `${task.name}_cv_${f}`)
    ...
    const accMatch = result.stdout.match(/Validation Accuracy:\s+([\d.]+)/)
...
`
  ...
  const modelPath = join('models', taskName, 'model.pkl')
  const result = await new Promise((resolve, reject) => {
    let stdout = ''
    const proc = spawn('python3', ['-c', script, modelPath, String(topN)], {
      cwd: join(import.meta.dir, '..')
    })
    proc.stdout.on('data', d => { stdout += d.toString() })
    proc.stderr.on('data', () => {})
...
function calibrationBins(predictions, actual, { nBins = 10 } = {}) {
  ...
  for (const bin of bins) {
    if (bin.predictions > 0) {
      ...
      bin.avgConfidence = (bins.indexOf(bin) + 0.5) / nBins
