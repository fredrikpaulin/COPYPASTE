...
function augment(data, { synonymProb = 0.3, insertProb = 0.15, multiplier = 2 } = {}) {
  ...
  for (const row of data) {
    ...
    for (let i = 1; i < multiplier; i++) {
      let text = row.text
      if (Math.random() < 0.6) text = synonymReplace(text, synonymProb)
      if (Math.random() < 0.4) text = randomInsert(text, insertProb)
      if (text !== row.text) {
...
async function filterByConfidence(data, task, { apiKey, model, threshold = 0.7, onProgress }) {
  // Ask Claude to rate each example's quality/confidence
  const batchSize = 20
  ...
  for (let i = 0; i < batches; i++) {
    ...
    const prompt = `Rate the quality of each training example below for a ${task.type} task: "${task.description}".
