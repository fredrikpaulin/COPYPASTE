...
function crfMargin(tokens, model) {
  ...
  // Get best path score
  const best = viterbi(tokens, tags, weights, hashSize)

  // Find second-best final tag score from the DP table
  // We approximate by running viterbi and looking at the final position scores
  const n = tokens.length
  if (n === 0) return { margin: 0, bestScore: 0 }

  // Re-run partial viterbi to get all final scores
  const dp = Array.from({ length: n }, () => new Float64Array(tags.length))

  // Position 0
  for (let t = 0; t < tags.length; t++) {
    const feats = crfExtractFeatures(tokens, 0, '<S>')
    dp[0][t] = crfScore(feats, tags[t], weights, hashSize)
  }
  // Fill forward
  for (let i = 1; i < n; i++) {
    for (let t = 0; t < tags.length; t++) {
      ...
      for (let p = 0; p < tags.length; p++) {
        const feats = crfExtractFeatures(tokens, i, tags[p])
        const s = dp[i - 1][p] + crfScore(feats, tags[t], weights, hashSize)
  ...
  return { margin, bestScore: best.score }
}
